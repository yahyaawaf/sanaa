document.addEventListener('DOMContentLoaded', () => {
    ...
    // عداد زيارات محلي متوافق مع GitHub Pages.
    // ملاحظة: GitHub Pages استضافة ثابتة ولا تدعم PostgreSQL أو Express مباشرة.
    const VISITOR_STORAGE_KEY = `sanaa-mashor:visits:${window.location.pathname}`;

    const updateVisitorCounter = () => {
        try {
            const currentCount = Number.parseInt(localStorage.getItem(VISITOR_STORAGE_KEY) || '0', 10);
            const nextCount = Number.isFinite(currentCount) ? currentCount + 1 : 1;
            localStorage.setItem(VISITOR_STORAGE_KEY, String(nextCount));

            if (visitorCountElement) {
                visitorCountElement.textContent = nextCount.toLocaleString('ar-SA');
            }
        } catch (error) {
            console.error('Error updating local visitor count:', error);
        ...
        cards.forEach(card => {
            ...
            if (toggleBtn && municipalityList) {
                toggleBtn.addEventListener('click', () => {
                    ...
                    if (dayWeekList) dayWeekList.style.display = 'none';
                });
            }
